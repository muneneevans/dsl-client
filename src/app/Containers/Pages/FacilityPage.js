...
class FacilityScreen extends Component {
	...
	componentDidMount() {
		...
		this.props.facilityActions.changeFacilityInformationType(
			facilityInformationType.SUMMARY
		)
	...
	handleDataTabChange(e, data) {
		...
		case 1:
			this.props.facilityActions.changeFacilityInformationType(
				facilityInformationType.LIST
			)
			break
		case 2:
			this.props.facilityActions.changeFacilityInformationType(
				facilityInformationType.MAP
			)
			break
		default:
			break
	...
	render() {
		const levelPanes = [
			...
			{
				menuItem: "Constituencies",
				render: () => (
					<Tab.Pane attached={true}>
						<ConstituencyForm
							countyCodes={this.props.countyCodes}
							fetchCountyConstituencyCodes={
								this.props.commonActions.fetchCountyConstituencyCodes
							}
							constituencyCodesIsFetched={this.props.constituencyCodesIsFetched}
							constituencyCodes={this.props.constituencyCodes}
							fetchConstituencyWardCodes={
								this.props.commonActions.fetchConstituencyWardCodes
							}
							fetchConstituencyFacilities={
								this.props.facilityActions.fetchConstituencyFacilities
							}
				...
				render: () => (
					<Tab.Pane attached={true}>
						<WardForm
							...
							fetchWardFacilities={
								this.props.facilityActions.fetchWardFacilities
							}
		...
		const dataPanes = [
								...
								<Grid.Row stretched centered>
									<Segment>
										...
										<StackedBarChart
											...
											dataExists={this.props.countySummaryIsFetched}
											data={
												this.getSummary().summaryChartData.facilitiesSummary
											}
			...
			},
			{
				menuItem: "Map View",
				render: () => (
					<MapChart height={500} width={500} data={this.props.countrySummary} />
				)
			}
		]
				...
				<Grid columns="equal" padded stretched>
					...
					<Grid.Column stretched>
						<Grid.Column stretched>
							...
							<Grid.Column>
								<Tab
									menu={{ secondary: true, color: "green" }}
									panes={dataPanes}
									onTabChange={this.handleDataTabChange.bind(this)}
								/>
							</Grid.Column>
						</Grid.Column>
					</Grid.Column>
	...
	renderLoading() {
		return <Segment loading size="large" />
	}
}
...
const mapStateToProps = (state, ownProps) => {
	return {
		...
		constituencyCodesIsFetched: commonSelectors.getCountyConstituencyCodesFetchStatus(
			state
		),
		...
		countySummaryIsFetched: facilitySelectors.getCountySummaryFetchStatus(
			state
		),
		countySummaryChartData: facilitySelectors.getCountySummaryXYData(state),

		constituencySummaryIsFetched: facilitySelectors.getConstituencySummaryFetchStatus(
			state
		),
		constituencySummaryChartData: facilitySelectors.getConstituencySummaryChartData(
			state
		),
		...
		currentFacilityInformationType: facilitySelectors.getCurrentFacilityInformationType(
			state
		)
	}
}
