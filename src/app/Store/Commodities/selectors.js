export function getProductOptions(state) {
	if (state.commodityReducer.products) {
		let products = []
		state.commodityReducer.products.map((product) => {
			products.push({
				key: product.id,
				value: product.id,
				text: product.name
...
export function getFacilityProductOptions(state) {
	if (state.commodityReducer.facilityProducts) {
		...
		state.commodityReducer.facilityProducts.map(product => {
			products.push({
				key: product.id,
				value: product.id,
				text: product.name
...
export function getFacilityYearProductGraphs(state) {
	if (state.commodityReducer.facilityYearProducts) {
		let months = [
			...new Set(state.commodityReducer.facilityYearProducts.map(a => a.month))
		]
		let output = []
		let keys = []
		let monthDict = {
		...
		months.map(month => {
			...
			var foundProducts = state.commodityReducer.facilityYearProducts.filter(
				obj => {
